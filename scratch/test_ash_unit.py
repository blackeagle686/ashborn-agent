...
class TestSendCompletionRequest(unittest.TestCase):
    ...
    def test_empty_payload(self, mock_post):
        ...
        # Act
        result = send_completion_request(self.base_url, empty_payload)
    ...
    @patch('requests.post')
    def test_none_payload(self, mock_post):
        """Test handling of None payload."""
        # Arrange
        expected_response = {"error": "No content provided"}
        mock_response = MagicMock()
        mock_response.json.return_value = expected_response
        mock_response.raise_for_status.return_value = None
        mock_post.return_value = mock_response
        ...
        # Act
        result = send_completion_request(self.base_url, None)
    ...
    def test_environment_variable_loading(self):
        """Test that environment variables can be loaded (integration test)."""
        # This is more of an integration test to verify env vars work
        # In real usage, this would load from environment
        os.environ['ASHBORN_API_KEY'] = 'test-api-key'
        api_key = os.getenv('ASHBORN_API_KEY')
        self.assertEqual(api_key, 'test-api-key')
        
        # Clean up
        del os.environ['ASHBORN_API_KEY']
